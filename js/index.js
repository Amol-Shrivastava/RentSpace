...
const loginModal = document.querySelector("#userModal");
const loginbtn = document.querySelector(".user-icon");
...
const sampleSpaceObserver = new IntersectionObserver(
  (entries, sampleSpaceObserver) => {
    entries.forEach((entry) => {
      if (!entry.isIntersecting) {
        return;
      } else {
